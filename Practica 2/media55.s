# Práctica 5.5 - Media y resto de N enteros calculada en 32 y en 64 bits
...
formato:	.ascii "media \t = %11d \t resto \t = %11d    \n"
			.asciz 		 "\t = 0x %08x \t 	 \t = 0x %08x \n"
formatq:	.ascii "(64 bits) media \t = %11d \t resto \t = %11d    \n"
			.asciz 		 "\t = 0x %08x \t 	 \t = 0x %08x \n"
...
bucleq:
	mov  (%rbx, %rsi, 4), %eax	# Leemos el elemento en %eax
	cdqe						# Extensión de signo sobre %rax
	add  %rax, %rdi				# Acumulamos sobre %rdi
	mov  %rdi, %rax
	cqo							# Extensión de signo sobre %rdx
	idivq %rcx, %rax
	inc  %rsi
	cmp  %rsi, %rcx
	jne   bucleq
